...
Resource hierarchy uses only Argo CD `ParentRefs`; network topology uses only `NetworkingInfo`. ReleaseHub resolves explicit target references and selector/resource labels returned by Argo CD. When a reference omits group or version, a relationship is created only if the remaining identity fields resolve to one resource. The unavailable warning means no relationship evidence was returned. When some references or selectors cannot be resolved safely, ReleaseHub keeps the proven subgraph and shows an unresolved warning instead of inferring links from names or Kubernetes conventions. Active deployments refresh every five seconds. A terminal Request shows current live state rather than an immutable snapshot of that execution; the Request revision, digest, and result evidence remain the governance record.
...
If a page opened after a Web release shows “ReleaseHub has been updated,” the old page requested an asset that is no longer available. The client reloads automatically at most once for the same build; after another failure, it preserves the sidebar and a Reload button. Check that the Web service serves the new `index.html` and hashed assets before retrying. Clearing the browser cache is not the only recovery method, and this asset error does not mean that the Deployment execution failed.
